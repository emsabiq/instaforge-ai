...
async function validateToken(accessToken = config.instagram.accessToken) {
  try {
...
export async function ensureFreshInstagramToken() {
  ...
  if (!config.instagram.igUserId || !config.instagram.accessToken) {
    return { checked: false, refreshed: false, reason: "instagram_missing_config" };
  }
  ...
  try {
    ...
    if (isExpiredTokenError(error)) {
      throw new Error("INSTAGRAM_ACCESS_TOKEN sudah expired. Update secret sebelum publish.");
    }
    throw error;
  }
  ...
  if (debug?.expiresAt) {
    ...
    if (daysLeft > config.meta.tokenRefreshBeforeDays) {
      ...
      return { checked: true, refreshed: false, expiresAt: debug.expiresAt.toISOString(), ...validation };
    }
  }
  ...
  try {
    ...
    return { checked: true, refreshed: false, reason: "refresh_failed_but_current_token_valid", ...validation };
  }
}
